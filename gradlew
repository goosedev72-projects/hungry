...
if ! "$cygwin" && ! "$darwin" ; then
    case $MAX_FD in
      max*)
        ...
        # the limit is only increased if increasing it succeeded initially.
        MAX_FD=`ulimit -H -n` &&
        ulimit -n $MAX_FD ||
...
# Set the APP_HOME variable to the parent directory of the script
APP_HOME=`cd "$(dirname "$0")" && pwd -P`

# Create the classpath with the wrapper jar
CLASSPATH="$APP_HOME/gradle/wrapper/gradle-wrapper.jar"
...
# Execute the JVM with the options set
